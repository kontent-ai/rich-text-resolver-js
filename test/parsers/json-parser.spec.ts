import { nodeParse, browserParse } from "../../src";
...
describe("node parser", () => {
  ...
  it("parses item links", () => {
    const value = `<p><a data-item-id=\"23f71096-fa89-4f59-a3f9-970e970944ec\" href=\"\"><strong>link to an item</strong></a></p>`
    const result = nodeParse(value);
  ...
  it("parses external links", () => {
    const value = `<p>text<a href=\"http://google.com\" data-new-window=\"true\" title=\"linktitle\" target=\"_blank\" rel=\"noopener noreferrer\">external link/a></p>`
    const result = nodeParse(value);
  ...
  it("parses nested styles", () => {
    const value = `<p><strong>all text is bold and last part is </strong><em><strong>also italic and this is also </strong></em><em><strong><sup>superscript</sup></strong></em></p>`
    const result = nodeParse(value);
  ...
  it("parses lists", () => {
    const value = `<ul><li>first level bullet</li><li>first level bullet</li><ol><li>nested number in bullet list</li></ol></ul><ol><li>first level item</li><li>first level item</li><ol><li>second level item</li><li><strong>second level item </strong></li></ol>`;
    const result = nodeParse(value);
  ...
  it("parses linked items/components", () => {
    const value = "<object type=\"application/kenticocloud\" data-type=\"item\" data-rel=\"link\" data-codename=\"test_item\"></object>";
    const result = nodeParse(value);
