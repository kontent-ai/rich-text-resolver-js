...
describe("node parser", () => {
  it("parses empty rich text", () => {
    const result = nodeParse(dummyRichText.value);
  ...
  it("parses external links", () => {
    dummyRichText.value = `<p>text<a href=\"http://google.com\" data-new-window=\"true\" title=\"linktitle\" target=\"_blank\" rel=\"noopener noreferrer\">external link/a></p>`
    const result = nodeParse(dummyRichText.value);
  ...
  it("creates identical output as browser parser", () => {
    const nodeParseResult = nodeParse(dummyRichText.value);
    const browserParseResult = browserParse(dummyRichText.value);
